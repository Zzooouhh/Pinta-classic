...
namespace Pinta.Gui.Widgets
{
	public class CanvasRenderer
	{
		...
		public void Render (List<Layer> layers, Cairo.ImageSurface dst, Point offset)
		{
			...
			using (var g = new Cairo.Context (dst)) {
				...
				for (var i = 0; i < layers.Count; i++) {
                    ...
                    // Fix to make pasted region non-transparent
                    var doc = PintaCore.Workspace.ActiveDocument;

                    if (doc.ShowSelectionLayer && layer == doc.CurrentUserLayer) {

                        // Draw everywhere except selected region
                        g.Rectangle (0, 0, source_size.Width, source_size.Height);
                    ...
                    using (var src_pattern = new Cairo.SurfacePattern(surf)) {
                        src_pattern.Filter = filter;
		...
		private int[] D2SLookupX => d2sLookupX = d2sLookupX ?? CreateLookupX (source_size.Width, destination_size.Width, scale_factor);
		private int[] D2SLookupY => d2sLookupY = d2sLookupY ?? CreateLookupY (source_size.Height, destination_size.Height, scale_factor);
		private int[] S2DLookupX => s2dLookupX = s2dLookupX ?? CreateS2DLookupX (source_size.Width, destination_size.Width, scale_factor);
		private int[] S2DLookupY => s2dLookupY = s2dLookupY ?? CreateS2DLookupY (source_size.Height, destination_size.Height, scale_factor);
